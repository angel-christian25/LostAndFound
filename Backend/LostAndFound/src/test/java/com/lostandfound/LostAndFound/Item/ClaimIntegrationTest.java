...
public class ClaimIntegrationTest {
    ...
    this.mockMvc
        .perform(
            put("/items/claims/revoke")
                ...
                .param("itemId", foundItem.getId()))
        .andExpectAll(
            status().isOk(), MockMvcResultMatchers.jsonPath("$.claimRequested.def").doesNotExist());
  }
  ...
  public void testApproveClaimSuccess() throws Exception {
    ...
    this.mockMvc
        ...
        .andExpectAll(
            ...
            MockMvcResultMatchers.jsonPath("$.claimRequestAccepted.def")
                .value(lostItem.getCreatedBy()),
            MockMvcResultMatchers.jsonPath("$.claimRequestRejected.def").doesNotExist());
  }
  ...
  public void testRejectClaimSuccess() throws Exception {
    ...
    this.mockMvc
        ...
        .andExpectAll(
            status().isOk(),
            MockMvcResultMatchers.jsonPath("$.claimRejected.def").value(lostItem.getCreatedBy()),
            MockMvcResultMatchers.jsonPath("$.claimRequestAccepted.def").doesNotExist());
  }
  ...
  public void testRejectClaimFailure() throws Exception {
    ...
    this.mockMvc
        ...
        .andExpectAll(
            status().is4xxClientError(),
            MockMvcResultMatchers.jsonPath("$")
                .value("User's claim request has already been rejected for this item."));
  }
}
